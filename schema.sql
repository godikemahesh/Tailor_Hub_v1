...
DROP TABLE IF EXISTS design_showcase_posts CASCADE;
...
-- ==============================================================================
-- 8b. Tailor Records Table (Multi-Modal Intake & Registry)
-- ==============================================================================
CREATE TABLE IF NOT EXISTS tailor_records (
    ...
    notes TEXT,
    source VARCHAR(30) DEFAULT 'manual' CHECK (source IN ('manual', 'voice_ai', 'camera_ocr')),
    created_at TIMESTAMPTZ DEFAULT NOW(),
    updated_at TIMESTAMPTZ DEFAULT NOW()
);

CREATE INDEX IF NOT EXISTS idx_tailor_records_tailor ON tailor_records(tailor_id);
CREATE INDEX IF NOT EXISTS idx_tailor_records_phone ON tailor_records(customer_phone);
CREATE INDEX IF NOT EXISTS idx_tailor_records_code ON tailor_records(customer_code);
...
-- ==============================================================================
-- 10. Design Showcase Posts Table (Master Tailor Showcase Feed)
-- ==============================================================================
CREATE TABLE design_showcase_posts (
    id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    tailor_id UUID NOT NULL REFERENCES users(id) ON DELETE CASCADE,
    shop_id UUID REFERENCES tailor_shops(id) ON DELETE CASCADE,
    title VARCHAR(200) NOT NULL,
    description TEXT NOT NULL,
    garment_category VARCHAR(50) NOT NULL,
    fabric_details VARCHAR(255),
    turnaround_days INT DEFAULT 4,
    estimated_price NUMERIC(10,2),
    image_url TEXT NOT NULL,
    tags TEXT[] DEFAULT ARRAY['bespoke', 'handcrafted'],
    likes_count INT DEFAULT 0,
    created_at TIMESTAMPTZ DEFAULT NOW(),
    updated_at TIMESTAMPTZ DEFAULT NOW()
);

CREATE INDEX idx_design_posts_tailor ON design_showcase_posts(tailor_id);
CREATE INDEX idx_design_posts_category ON design_showcase_posts(garment_category);
